...
class TestService(unittest.TestCase):
    ...
    def test_addition(self, mock_create):
        response = self.client.post('/calculate', json={'a': 5, 'b': 3, 'operation': 'add'})
    ...
    def test_division_by_zero(self, mock_create):
        response = self.client.post('/calculate', json={'a': 5, 'b': 0, 'operation': 'divide'})
        data = json.loads(response.data)
    ...
    def test_get_history(self, mock_get_history):
        mock_history = [
            {'operand1': 5, 'operand2': 3, 'operation': 'add', 'result': 8, 'created_at': '2023-08-08 12:00:00'}
        ]
